...
class BasicTrainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        # Optional CUDA cache cleanup to mitigate allocator fragmentation / runaway reserved memory.
        self.cuda_empty_cache_interval = int(kwargs.get('cuda_empty_cache_interval', 0) or 0)
        self.cuda_empty_cache_reserved_gb = kwargs.get('cuda_empty_cache_reserved_gb', None)
        self.cuda_empty_cache_free_gb = kwargs.get('cuda_empty_cache_free_gb', None)
        self.cuda_empty_cache_ratio = kwargs.get('cuda_empty_cache_ratio', None)
        self.cuda_ipc_collect = bool(kwargs.get('cuda_ipc_collect', False))
        if self.cuda_empty_cache_reserved_gb is not None:
            self.cuda_empty_cache_reserved_gb = float(self.cuda_empty_cache_reserved_gb)
        if self.cuda_empty_cache_free_gb is not None:
            self.cuda_empty_cache_free_gb = float(self.cuda_empty_cache_free_gb)
        if self.cuda_empty_cache_ratio is not None:
            self.cuda_empty_cache_ratio = float(self.cuda_empty_cache_ratio)
    ...
    def init_models_and_more(self, **kwargs):
        ...
        if self.world_size > 1:
            # Prepare distributed data parallel (skip models with no trainable parameters)
    ...
    @torch.no_grad()
    def snapshot(self, suffix=None, num_samples=1, batch_size=4, verbose=False, **kwargs):
        """
    ...
    def check_ddp(self):
        ...
        dist.barrier()
        # Old behavior hard-failed on bitwise mismatch:
        ...
        # For long AMP/DDP sparse-conv runs, treat this health check as a repair
        # point: report any drift, then broadcast rank0 weights so all ranks
        # continue from the same parameters.
        mismatch_count = 0
        max_abs_diff = 0.0
        for p in self.master_params:
            # split to avoid OOM
            flat_p = p.detach().view(-1)
            for i in range(0, p.numel(), 10000000):
                sub_size = min(10000000, p.numel() - i)
                sub_p = flat_p[i:i+sub_size]
                # gather from all processes
                sub_p_gather = [torch.empty_like(sub_p) for _ in range(self.world_size)]
                dist.all_gather(sub_p_gather, sub_p)
                for other in sub_p_gather:
                    if not torch.equal(sub_p, other):
                        mismatch_count += 1
                        diff = (sub_p.float() - other.float()).abs().max().item()
                        max_abs_diff = max(max_abs_diff, diff)

        if self.is_master:
            print(f"[DDPCHECK] mismatch_chunks={mismatch_count} max_abs_diff={max_abs_diff:.3e}", flush=True)
        if mismatch_count > 0:
            if self.is_master:
                print(
                    f'\n\033[93mWarning: DDP parameter mismatch detected '
                    f'(chunks={mismatch_count}, max_abs_diff={max_abs_diff:.6e}). '
                    f'Broadcasting rank0 parameters and continuing.\033[0m'
                )
            dist.barrier()
            for p in self.master_params:
                flat_p = p.detach().view(-1)
                for i in range(0, p.numel(), 10000000):
                    sub_size = min(10000000, p.numel() - i)
                    dist.broadcast(flat_p[i:i+sub_size], src=0)
    ...
    def run_step(self, data_list):
        ...
        if self.mix_precision_mode == 'amp' and self.mix_precision_dtype == torch.float16:
            ...
            if self.mix_precision_dtype == torch.float16:
                ...
                if not any((p.grad is not None) and (not torch.isfinite(p.grad).all()) for p in self.model_params):
                ...
                if self.grad_clip is None:
                    model_grads_to_master_grads(self.model_params, self.master_params)
                if not any((p.grad is not None) and (not torch.isfinite(p.grad).all()) for p in self.master_params):
                    self.optimizer.step()
                    master_params_to_model_params(self.model_params, self.master_params)
                else:
    ...
    def _maybe_cleanup_cuda_cache(self, *, force: bool = False) -> None:
        if not torch.cuda.is_available():
            return

        do = force
        if not do and self.cuda_empty_cache_interval and self.step % self.cuda_empty_cache_interval == 0:
            do = True

        if not do and (self.cuda_empty_cache_reserved_gb is not None or self.cuda_empty_cache_free_gb is not None or self.cuda_empty_cache_ratio is not None):
            alloc_gb = torch.cuda.memory_allocated() / 1e9
            rsvd_gb = torch.cuda.memory_reserved() / 1e9
            free_gb = rsvd_gb - alloc_gb
            ratio = rsvd_gb / max(alloc_gb, 1e-6)

            if self.cuda_empty_cache_reserved_gb is not None and rsvd_gb >= self.cuda_empty_cache_reserved_gb:
                do = True
            if not do and self.cuda_empty_cache_free_gb is not None and free_gb >= self.cuda_empty_cache_free_gb:
                do = True
            if not do and self.cuda_empty_cache_ratio is not None and ratio >= self.cuda_empty_cache_ratio:
                do = True
    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            # Cleanup before allocations in this step (helps with fragmentation).
            self._maybe_cleanup_cuda_cache()
            ...
            #     sizes = []
            #     for mb in data_list:
            #         if isinstance(mb, dict) and 'vertices' in mb and hasattr(mb['vertices'], 'feats'):
            #             sizes.append(int(mb['vertices'].feats.shape[0]))
                # if len(sizes) > 0:
                #     print('vertices_points', sizes, flush=True)

            # if self.is_master and torch.cuda.is_available():
            #     alloc = torch.cuda.memory_allocated() / 1e9
            #     rsvd = torch.cuda.memory_reserved() / 1e9
            #     print('allocGB', alloc, 'reservedGB', rsvd, flush=True)

            # Cleanup after the step if allocator reserved memory is disproportionate.
            self._maybe_cleanup_cuda_cache()
            ...
            if self.is_master and self.step % self.i_print == 0:
                ...
                if hasattr(self, 'optimizer'):
                    try:
                        ...
                        print(
                            "step: ", self.step,
                            'loss: ', step_log['loss']['loss'].tolist(),
                            'bce_sub2: ', step_log['loss']['bce_sub2'].tolist(),
                            'render/normal/ssim: ', step_log['loss']['render/normal/ssim'].tolist(),
                            'vq/util_mean: ', step_log['loss']['vq/utilization_mean'].tolist() if 'vq/utilization_mean' in step_log['loss'] else None,
                            'direct/intersected: ', step_log['loss']['direct/intersected'].tolist() if 'direct/intersected' in step_log['loss'] else None,
                            'bce_sub3: ', step_log['loss']['bce_sub3'].tolist() if 'bce_sub3' in step_log['loss'] else None,
                            'kl: ', step_log['loss']['kl'].tolist() if 'kl' in step_log['loss'] else None,
                            'render/normal/lpips: ', step_log['loss']['render/normal/lpips'].tolist() if 'render/normal/lpips' in step_log['loss'] else None,
                            'bce_sub0: ', step_log['loss']['bce_sub0'].tolist() if 'bce_sub0' in step_log['loss'] else None,
                            'bce_sub1: ', step_log['loss']['bce_sub1'].tolist() if 'bce_sub1' in step_log['loss'] else None,
                            'lr: ', lrs,
                        )
                    except Exception:
                        pass 
