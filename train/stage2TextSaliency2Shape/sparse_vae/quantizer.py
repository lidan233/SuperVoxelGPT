"""Finite scalar quantization: the codebook the supervoxel tokens are drawn from.

Purpose
    The encoder produces one continuous vector per supervoxel; this turns each into one integer.
    That integer is the token the shape generator predicts, so the vocabulary here *is* the
    generator's alphabet -- `levels` multiplied out gives the 10125 the generator was trained
    against, and changing it invalidates every token ever extracted.

Input
    Supervoxel features, batched and padded to a rectangle by the caller.

Output
    The quantized features and their integer indices.
"""
