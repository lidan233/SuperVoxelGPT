#!/usr/bin/env python3
"""Train the saliency generator on captions — text2saliency.
"""
import argparse
import os
...
from ..dataset import text2saliency as text_ds
...
def build_dataset(args):
    ...
    ds = text_ds.TextToSaliencyDataset(
        code_dir=args.code_dir, condition_dir=args.condition_dir, grid=args.grid_size,
        code_key=args.code_key, max_samples=args.max_samples,
        repeat_factor=args.repeat_factor, train=True, allowed_ids=train_ids)
    eval_ds = None if not val_ids else text_ds.TextToSaliencyDataset(
        code_dir=args.code_dir, condition_dir=args.condition_dir, grid=args.grid_size,
...
def main():
    ap = argparse.ArgumentParser(description="Train the saliency generator (text2saliency, image2saliency).")
    add_config_flag(ap)
    ap.add_argument("--code-dir", required=True, help="per-object saliency codes")
    ap.add_argument("--condition-dir", required=True, help="precomputed condition features")
